...
在单入口架构中，用户默认从 `$lvsea-writing` 开始；`lvsea-xiezuo` 是按需调用的小红书素材雷达专家。它负责在用户有权处理的公开或自有数据中做来源校验、互动门禁、证据拆分、评论分析、去重和二创选题简报，然后停在“可审阅的写作输入”，不在没有任务契约、主判断和作者声音的情况下直接批量生成最终正文。
...
不要把两个 Skill 的规则复制成一个宽泛的“写作 Skill”：保留 `$lvsea-writing` 的单入口，把本 Skill 作为小红书素材前置专家按需路由，仍由 `$lvsea-writing` 负责通用写作和最后编辑。
